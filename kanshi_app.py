...
st.markdown("""
    ...
    h3 { text-align: center; font-size: 2rem !important; color: #2E4053; }
    </style>
...
if st.session_state.current_poem is not None:
    ...
    if st.session_state.hint_level >= 1: st.write(f"作者：[{p['dynasty']}] {p['author']}")
    if st.session_state.hint_level >= 2: st.info(f"第一句：{p['phrase_1']}")
    if st.session_state.hint_level >= 3: st.success(f"全文：\n\n{p['full_text']}")
    st.markdown('</div>', unsafe_allow_html=True)

    # 標準的なカラム：PCでは横、スマホでは自動で縦
...
# --- 6. 一覧表示 ---
st.divider()
st.subheader(f"📚 汉诗一览 ({len(filtered_df)}首)")
for index, row in filtered_df.iterrows():
    with st.expander(f"{row['id']}. {row['title']} / {row['author']}"):
        st.write(f"**朝代：** {row['dynasty']}  |  **分類：** {row['category']}")
